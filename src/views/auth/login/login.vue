	...
	export default {
		...
		methods: {
			...
			async login() {
				...
				// 获取token
				const login_token = await loginApi.login(loginData).catch(() => {
					this.loginCaptcha()
					this.islogin = false
				})
				this.$TOOL.data.set('TOKEN', login_token)
				// 获取登录的用户信息
				...
				if(login_token){
					...
					// 获取用户的菜单
					var menu = await userCenterApi.userLoginMenu().catch(() => {
						this.islogin = false
						return
					})
				}
				...
				if (typeof menu !== 'undefined') {
					...
					const indexMenu = menu[0].children[0].children[0].path
