...
//双线节点索引
function findMutilNode (startTopologyNode,nodes) {
  for (let i = 0; i < nodes.length; i++){
    if (nodes[i].data.id == startTopologyNode.id) {
      // 网格布局
      const cols =2;
      const points = Shape.outerGrid(startTopologyNode.num, cols).points;
      // 坐标->Node对象
      function pointToNode(p, i) {
          let endpoint = new CircleNode('');
          return endpoint;
      ...
      const subNodes = points.map(pointToNode);
      nodes[i].addChilds(subNodes);
      nodes[i].setUserData({id: startTopologyNode.id,type:'node',ptype:startTopologyNode.type, name:startTopologyNode.name,children:startTopologyNode.children,points: points });
      return nodes[i];
    }
  }
}
...
class TopoManager{
  ...
  init (divObj, data, type) {
        ...
        stage.toolbar.getDom().querySelector('[title="导出PNG"]').remove();
        stage.toolbar.getDom().querySelector('[title="默认"]').remove();
        stage.toolbar.getDom().querySelector('[title="编辑模式"]').remove();
        stage.toolbar.getDom().querySelector('[title="框选模式"]').remove();
        stage.toolbar.getDom().querySelector('[title="拖拽模式"]').remove();
        stage.toolbar.getDom().querySelector('[title="锁定模式"]').remove();
        stage.toolbar.getDom().querySelector('[title="居中"]').remove();
        stage.toolbar.getDom().querySelector('[title="缩放至画布"]').remove();
        stage.toolbar.getDom().querySelector('[title="取消缩放"]').remove();
        stage.toolbar.getDom().querySelector('[title="缩略图"]').remove();
        stage.toolbar.getDom().querySelector('[title="浏览器全屏"]').remove();
        stage.toolbar.getDom().setAttribute('style','display:none')
        
        // console.log(stage.toolbar.getDom().setAttribute('style','display:none'))
        // stage.toolbar.getDom().querySelector('.jtopo_toolbar').setAttribute('style','display:none');
        layer.zoom(0.1,0.1)
      ...
      if(l_data){
        for (var i = 0; i < l_data.length; i++){
          if(l_data[i].num < 2){
            ...
            drawLinks(findMutilNode(l_data[i].startTopologyNode,nodes), findMutilNode(l_data[i].endTopologyNode,nodes));
          }
    ...
    function drawLinks(nodeA,nodeZ){
      ...
      for (let i = 0; i < outputs.length; i++) {
        ...
        // let toIndex = i * (inputs.length-2);
          ...
          let from = outputs[i];
          let to = inputs[i];
          if((!from.parent.data.children[i]) || (!to.parent.data.children[i])){
            continue;
          }
