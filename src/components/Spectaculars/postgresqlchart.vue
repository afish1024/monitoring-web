    ...
    <div class="content" v-if="isData" style="height: 400px;">
        <a-row :gutter="40" justify="space-between" class="contdiv">
            <a-col :span="24" class="title" v-if="hostsinfo">
               {{ hostsinfo }} : Postgresql数据库
            </a-col>
        </a-row>
        <a-row :gutter="40" justify="space-between" class="contdiv">
            <a-col :span="24" class="leftcontent" >
                <div id="postgresqlChart" style="height:280px;width:98%" ></div> 
            </a-col>
        </a-row>
    </div> 

</template>
<script setup name="postgresqlChart">
    ...
    let isData = ref(true)
    const toRefStrHtml = toRef(props, "postgresqlData")
    watch(toRefStrHtml, () => {
    ...
    let getData= (datalist) => {
        ...
        for(var i in datalist) {
            ...
            for(let r = 0; r < resdata.length; r++) {
                var dataValue = resdata[r].value;
                if(i == 'size'){
                    dataValue = (dataValue/1024).toFixed(2)
                }
                data.push(dataValue)    
                if(i == 'size'){
        ...
        let Echarts =  echarts.init(document.getElementById('postgresqlChart'))
