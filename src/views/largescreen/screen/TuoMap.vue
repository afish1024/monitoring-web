<template>
      <!-- <div style="cursor: pointer; ">
        <img :src="server" style="width: 20px;height: 20px;"/>
      </div> -->
      <div id="divId" style="height:calc(60vh - 50px); width: 150%;position: relative;bottom: 10%;"></div>
      <component :is="currentComponent" :par="par"></component>
  </template>
  <script setup name="topologyList">
  import { topoManager } from "./TopoManager";
  import topologyApi from '@/api/topology/topologyApi';
  import exchange from '../../topology/exchange/index.vue'
  ...
  onMounted(() => {
    // loadG6Data()
    topologyApi.topology({ id: props.id }).then((res1) => {
      topoManager.init(document.getElementById("divId"), res1,'child')
    ...
    //   console.log(data)
    //   par.value.id = data.id
    //   par.value.networkName = data.networkName
