				...
				<header id="snowyHeaderLogo" class="snowy-header-logo">
					<div class="snowy-header-left">
						<div class="logo-bar">
							...
							<span style="font-size: 12px;">{{ sysBaseConfig.SNOWY_SYS_NAME }}</span>
						</div>
					</div>
				</header>
		...
		created() {
			...
			if(menu.length > 0){
				menu.map((item)=>{
					...
					if(item.path != '/usercenter'){
						newMenu.push(item)
					}
				})
			}
			console.log(menu)
			this.menu = this.filterUrl(newMenu)
			this.showThis()
		},
		mounted() {
			console.log(this.layoutSiderDowbleMenu)
			this.switchoverTopHanderThemeColor()
		},
		methods: {
			// 切换应用
