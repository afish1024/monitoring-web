...
import { PointLike, StyleOpt, Vec2 } from './jtopo_type';
...
export declare abstract class DisplayObject extends EventTarget {
    ...
    name?: string;
    /**
    * 类型，自定义类型，可以用于自定义节点，区分节点类型, 比用data属性更方便一点
    * @since 2.4.0
    */
    type?: string;
    z: number;
    ...
    *
```js
    node.paintSelected = function(ctx){
        let lineWidth = 3;
        let w = this.width + lineWidth + 3;
        let h = this.height + lineWidth + 3;
        ctx.save();
...
```
     * @param  styleOption
     */
    css(styleOption: StyleOpt | string, value?: any): this;
    /**
     * 完全等价于css 方法
     */
    setStyles(styleOpt: StyleOpt, value?: any): this;
    /**
...
```
     ...
     * @param  y
     */
    toLayerXY(x: number, y: number): any;
    /**@protected */
...
```
     ...
     */
    destory(): void;
    static flatten(arr: Array<any>, cond?: Function, breadthFirst?: boolean): any[];
    static getNoChildrensObjects(arr: Array<DisplayObject>): any[];
    /**@protected */
    static _anyMatrixDirty(obj: DisplayObject): boolean;
    set zIndex(v: number);
    get zIndex(): number;
    get x(): number;
    set x(v: number);
