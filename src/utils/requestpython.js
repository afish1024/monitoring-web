...
service.interceptors.request.use(
	(config) => {
		...
		if(userInfo){
			...
			config.headers['name'] = encodeURI(userInfo.name)
		}
	...
	(response) => {
		...
		if (code !== 200) {
			...
			const apiNameArray = ['add', 'edit', 'delete', 'update', 'grant', 'reset', 'start', 'stop',
				'pass', 'disable', 'enable', 'revoke', 'suspend', 'active', 'turn', 'adjust', 'reject','sendsms','createhost','hostmonitor','hostset','hosttype','monitorstatus']
...
export const baseRequest = (url, value = {}, method = 'post', options = {}) => {
	url = sysConfig.API_PYTHON_URL + url
	if (method === 'post') {
