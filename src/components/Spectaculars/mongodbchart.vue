    ...
    <div class="content" v-if="isData" style="height: 400px;">
        <a-row :gutter="40" justify="space-between" class="contdiv">
            <a-col :span="24" class="title" v-if="hostsinfo">
               {{ hostsinfo }} : Mongodb数据库
            </a-col>
        </a-row>
        <a-row :gutter="40" justify="space-between" class="contdiv">
            <a-col :span="24" class="leftcontent" >
                <div  id="mongodbChart" style="height:280px;width:98%" ></div> 
            </a-col>
        </a-row>
    </div> 

</template>
<script setup name="mongodbChart">
    ...
    let isData = ref(true)
    const toRefStrHtml = toRef(props, "mongodbData")
    watch(toRefStrHtml, () => {
    ...
    let getData= (datalist) => {
        ...
        let Echarts =  echarts.init(document.getElementById('mongodbChart'))
        // 绘制图表
