<template>
	<div class="user-bar">
		...
		<!-- <a-tooltip>
			<template #title>{{ mutemsg }}</template>
			<div v-if="ismute" @click="muteClick" class="pointer">
			
				<audio-outlined />
			</div>
			<div v-else @click="muteClick"  class="pointer">
	...
	export default {
		...
		data() {
			return {
				...
				userName: '',
				userNameF: '',
				mutemsg:'当前已开启页面告警声音提示，点击可关闭'
			}
		},
		computed: {
			ismute() {
				return this.$store.state.global.ismute
			},
			ismobile() {
		...
		methods: {
			muteClick(){
				this.$store.commit('SET_ismute', !this.$store.state.global.ismute)
				if(this.$store.state.global.ismute){
					this.mutemsg = '当前已开启页面告警声音提示，点击可关闭'
				}else{
					this.mutemsg = '当前已关闭页面告警声音提示，点击可开启'
				}
			},
			// 个人信息
			handleUser(key) {
